...
void guardado(){
	...
	if(X==NULL){
		...
		fprintf(X,"%d",dat[i].matricula);
		fprintf(X,"---");
			fprintf(X,"%s",dat[i].nom_completo);
		fprintf(X,"---");
		fprintf(X,"%d",dat[i].dia);
		fprintf(X,"/");
	    	fprintf(X,"%s",dat[i].mes);
		fprintf(X,"/");		
		fprintf(X,"%d",dat[i].year);	
		fprintf(X,"--");
		fprintf(X,"%d",dat[i].grado);
		fprintf(X," grado");
		fprintf(X,"---");	
		fprintf(X,"%s",dat[i].carrera);
		fprintf(X,"---");		
			fprintf(X,"%s",dat[i].escuela);
		fprintf(X,"---");		
		fprintf(X,"%s",dat[i].sexo);		
		fprintf(X,"\n");
	}
	fclose(X);
}
